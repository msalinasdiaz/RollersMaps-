...
type AdminEvent={id:string;title:string;status:string;starts_at:string};
export default function GroupScreen(){
  ...
  const loadAdmin=useCallback(async()=>{
    ...
    const [m,a]=await Promise.all([supabase.rpc('get_group_members',{p_group_id:id}),supabase.from('activities').select('id,title,status,starts_at').eq('group_id',id).order('starts_at',{ascending:false}).limit(100)]);
  ...
  return <Screen back title={group?.name??'Grupo'} subtitle={group?.city} refresh={()=>{void refresh();void loadAdmin();}} refreshing={false}>
    {!group?<Notice title={isLoading?'Cargando grupo…':'Grupo no disponible'} text={groupsError??'Vuelve al directorio e inténtalo nuevamente.'} action="Ver grupos" onAction={()=>router.replace('/groups')}/>:<>
      ...
      {section==='about'||!admin?<>
        <Card><Text style={ui.text}>{group.description||'Una comunidad para compartir el patinaje.'}</Text><Text style={ui.muted}>{group.member_count} miembros · {group.join_policy==='approval'?'Ingreso por aprobación':'Ingreso abierto'}</Text></Card>
