...
export default function HomeScreen() {
  ...
  const { groups,activities,calendarError,isPlatformAdmin,adminError,refresh }=useCommunity();
  ...
  return <Screen title={firstName?`Hola, ${firstName}`:'Tu próxima salida'} subtitle="Patina a tu ritmo. Encuentra tu comunidad.">
    {isPlatformAdmin?<Card><Text style={ui.heading}>Administración general</Text><Text style={ui.muted}>Revisa todos los grupos y aprueba o rechaza nuevas solicitudes.</Text><Button onPress={()=>router.push('/group-requests')}>Grupos y aprobaciones</Button></Card>:adminError?<Notice title="No pudimos verificar tu acceso administrativo" text={adminError} action="Reintentar" onAction={()=>void refresh()}/>:null}
