...
export default function GroupsScreen(){
  const {groups,isLoading,groupsError,refresh,isPlatformAdmin}=useCommunity();
  ...
  return <Screen title="Grupos" subtitle="Encuentra con quién patinar." refresh={()=>void refresh()} refreshing={isLoading&&groups.length>0}>
    {isPlatformAdmin?<Button secondary onPress={()=>router.push('/group-requests')}>Solicitudes de nuevos grupos</Button>:null}
