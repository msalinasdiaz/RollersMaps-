...
export default function GroupsScreen(){
  const {groups,isLoading,groupsError,refresh}=useCommunity();
  const {isSignedIn}=useDemoSession();
  const [query,setQuery]=useState('');
  const [mine,setMine]=useState(false);
  const shown=useMemo(()=>groups.filter(g=>(!mine||['active','pending'].includes(g.membership_status??''))&&`${g.name} ${g.city}`.toLocaleLowerCase().includes(query.toLocaleLowerCase().trim())),[groups,mine,query]);
