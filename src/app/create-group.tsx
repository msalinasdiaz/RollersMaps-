...
export default function CreateGroup(){
  ...
  async function save(){
    ...
    try{const {data,error}=await supabase.rpc('create_group',{p_name:name.trim(),p_description:description.trim(),p_city:city.trim(),p_join_policy:policy});
      if(error){setError('No pudimos crear el grupo. Revisa tu conexión e inicia sesión.');return;}
  ...
  return <Screen back title="Crea tu comunidad" subtitle="Serás el propietario y podrás designar administradores.">
    ...
    {error?<Text accessibilityLiveRegion="polite" style={ui.accent}>{error}</Text>:null}<Button busy={busy} onPress={()=>void save()}>Crear grupo</Button>
  </Screen>;
}
